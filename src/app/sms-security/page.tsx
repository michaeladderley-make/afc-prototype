...
import { SmsSecuritySetup } from "@/components/sms-security/sms-security-setup";
import { getSchoolById } from "@/lib/mock-schools";
import { partnerQuery } from "@/lib/partner-context";

export default async function SmsSecurityPage({
  searchParams,
}: PageProps<"/sms-security">) {
  const {
    email,
    type,
    school: schoolId,
  ...
  return (
    <div className="flex min-h-full flex-col bg-background">
      ...
      <main className="flex w-full flex-1 justify-center px-16 pt-24 pb-16">
        <SmsSecuritySetup email={workEmail} returnHref={returnHref} />
      </main>
    </div>
  );
}
